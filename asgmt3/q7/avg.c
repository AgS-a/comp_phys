...
double Cv(int L, double T)
{
        ...
        while(fscanf(fPtr1, "%lf", &entry) != EOF){
                i += 1;
                entry = entry * L * L; // Total energy
                sum += entry;
                sum_sq += entry*entry;
        }
        fclose(fPtr1);

        double cv = ( (sum_sq/i) - ( (sum/i)*(sum/i) ) )/(T*T);
        return cv;
}
...
double Cai(int L, double T)
{
        ...
        double cai = ( (sum_sq/i) - ( (sum/i)*(sum/i) ) )/T;
        return cai;
}
...
int main()
{
        ...
        FILE *file1;
        FILE *file2;
        file1 = fopen("cv_L26.dat","w");
        file2 = fopen("cai_L26.dat","w");
        ...
        FILE *file3;
        FILE *file4;
        file3 = fopen("cv_L30.dat","w");
        file4 = fopen("cai_L30.dat","w");
        ...
        FILE *file5;
        FILE *file6;
        file5 = fopen("cv_L36.dat","w");
        file6 = fopen("cai_L36.dat","w");
